...
description: Connect your coding agent to Autonoma's onboarding MCP server so it can configure and deploy your PreviewKit preview for you - reading the config, setting it up, asking you for secrets, deploying off main, and iterating until it comes up - while you watch read-only in Autonoma.
---

import Tabs from "../../../components/Tabs.astro"
import TabItem from "../../../components/TabItem.astro"

<p class="lead">Autonoma runs a second MCP server for onboarding: connect your coding agent and it configures your PreviewKit preview for you - reads the config, sets up apps, databases and services, asks you for any secrets it needs, deploys off your main branch, and iterates until the preview is up - while you watch, read-only, in the Autonoma UI.</p>
...
:::note
This is a different server from the [debugging MCP](/mcp). That one helps your agent fix a broken preview on an existing pull request; this one helps your agent do the initial PreviewKit setup during onboarding.
:::
...
1. In Autonoma, start onboarding your app and open the PreviewKit config step.
...
2. **Read and edit** - `get_config` / `apply_config` read and save the full PreviewKit config document. A save is validated; an invalid document returns the errors to fix.
