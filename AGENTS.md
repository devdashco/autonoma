...
**Always use components from `@autonoma/blacklight` when building frontend interfaces.** This is a shared component library built on Radix UI + Tailwind CSS v4 + CVA. See `.Codex/skills/design.md` for the full design system reference and `.Codex/skills/frontend-design.md` for high-quality frontend design guidelines.
...
22. **On compaction, re-read documentation.** When context compaction occurs, immediately re-read any relevant documentation files (AGENTS.md, prompts, docs) before continuing work. Use a TODO list to track what has been done and what remains. This prevents losing track of progress or requirements after compaction.
